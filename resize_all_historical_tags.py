...
from secrets import DATABASE, RESIZE_ALL_HISTORICAL_TAGS_TEMP_DIR
...
files = glob.glob('{0}*'.format(RESIZE_ALL_HISTORICAL_TAGS_TEMP_DIR))
for f in files:
    os.remove(f)
...
for tag in tags:
    ...
    if tag.image_id in db_images:
        ...
        local_file = get_file_name(RESIZE_ALL_HISTORICAL_TAGS_TEMP_DIR, db_image.large_thumbnail)
        ...
        local_file = download_file(RESIZE_ALL_HISTORICAL_TAGS_TEMP_DIR, db_image.large_thumbnail)
        print(db_image.title)
        print(local_file)
